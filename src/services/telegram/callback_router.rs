...
async fn handle_callback_data(
    ...
    match callback {
        ...
        // Регистрация
        Faculty(faculty) => {
            registration::handlers::handle_faculty_choice(bot, q, faculty).await?;
        }
        StudyForm(form) => {
            registration::handlers::handle_study_form(bot, q, form).await?;
        }
        Course(course) => {
            registration::handlers::handle_course(bot, q, course).await?;
        }
        MitGroup(group) => {
